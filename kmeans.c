...
int get_corres_cluster(double dist[][3],int, int, int);
...
int get_corres_cluster(double dist[][3],int i, int SIZE, int K) 
{
   int j;
   double m;
   m=dist[i][0];
...
int assign_clusters(float data[][2], float centroids[][2], int ncluster[], int SIZE, int K){
   int i,j;
   double dist[SIZE][K];	//an array to store the cluster of each datapoint
